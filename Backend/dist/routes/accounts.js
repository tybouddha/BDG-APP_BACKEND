...
router.get("/accounts", async (req, res) => {
    try {
        ...
        if (!JWT_SECRET) {
            res.status(500).json({
                result: false,
                message: "JWT_SECRET doit être défini dans les variables d'environnement.",
            });
            return;
        }
        let decoded;
        try {
            decoded = jsonwebtoken_1.default.verify(token, JWT_SECRET); // Décodage du token
        }
        catch (err) {
        ...
        const accountsQuery = await (0, database_1.query)("SELECT id, name, balance, currency, is_active FROM accounts WHERE user_id = $1", [userIdFromToken]);
...
router.post("/create", async (req, res) => {
    ...
    try {
        ...
        let decoded;
        try {
            decoded = jsonwebtoken_1.default.verify(token, JWT_SECRET); // Vérifie et décode le token
        }
        catch (err) {
            res.status(401).json({ result: false, message: "Token invalide." });
            return;
        ...
        const userExists = await (0, database_1.query)("SELECT * FROM users WHERE id = $1", [
            user_id,
        ]);
        ...
        const result = await (0, database_1.query)(`INSERT INTO accounts (user_id, name, balance, currency, is_active)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, user_id, name, balance, currency, is_active`, [user_id, name, balance, currency, is_active]);
        // 5. Réponse de succès
        res.status(201).json({
            result: true,
...
router.put("/update/:id", async (req, res) => {
    ...
    try {
        ...
        const accountOwner = await (0, database_1.query)("SELECT user_id FROM accounts WHERE id = $1", [accountId]);
        ...
        const updatedAccount = await (0, database_1.query)(`UPDATE accounts 
       SET name = COALESCE($1, name), 
           balance = COALESCE($2, balance), 
           currency = COALESCE($3, currency), 
           is_active = COALESCE($4, is_active)
       WHERE id = $5
       RETURNING id, user_id, name, balance, currency, is_active`, [name, balance, currency, is_active, accountId]);
...
router.delete("/delete", async (req, res) => {
    try {
        ...
        if (id) {
            // Recherche par ID
            accountQuery = await (0, database_1.query)("SELECT id, user_id FROM accounts WHERE id = $1", [id]);
        }
        else if (name) {
            // Recherche par nom
            accountQuery = await (0, database_1.query)("SELECT id, user_id FROM accounts WHERE name = $1 AND user_id = $2", [name, userIdFromToken]);
        }
        ...
        const deleteAccount = await (0, database_1.query)("DELETE FROM accounts WHERE id = $1", [
            account.id,
        ]);
